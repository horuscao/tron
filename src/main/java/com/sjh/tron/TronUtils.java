...
public class TronUtils {
	 ...
	 * @return
	 */
	public static String signAndBroadcast(String tronUrl,String privateKey,JSONObject transaction, String filepath)throws Throwable{
		if(tronUrl.endsWith("/")){
		...
		WriteUtil.log("signAndBroadcast transationCompelet:" + transationCompelet.toJSONString(), filepath);
		if (transationCompelet.getBoolean("result")) {
			return transationCompelet.getString("txid");
		} else {
			WriteUtil.log(String.format("签名交易失败:%s",transationCompelet1), filepath);
			return null;
		}
	}
	 ...
	 * @return
	 */
	private static Protocol.Transaction packTransaction(String strTransaction) {
		...
		for (int i = 0; i < rawContractArray.size(); i++) {
			try {
				...
				Any any = null;
				if ("AccountCreateContract".equals(contractType)) {
				...
				} else if ("TransferContract".equals(contractType)) {
				...
				} else if ("TransferAssetContract".equals(contractType)) {
				...
				} else if ("VoteAssetContract".equals(contractType)) {
				...
				} else if ("VoteWitnessContract".equals(contractType)) {
				...
				} else if ("WitnessCreateContract".equals(contractType)) {
				...
				} else if ("AssetIssueContract".equals(contractType)) {
				...
				} else if ("WitnessUpdateContract".equals(contractType)) {
				...
				} else if ("ParticipateAssetIssueContract".equals(contractType)) {
				...
				} else if ("AccountUpdateContract".equals(contractType)) {
				...
				} else if ("FreezeBalanceContract".equals(contractType)) {
				...
				} else if ("UnfreezeBalanceContract".equals(contractType)) {
				...
				} else if ("UnfreezeAssetContract".equals(contractType)) {
				...
				} else if ("WithdrawBalanceContract".equals(contractType)) {
				...
				} else if ("UpdateAssetContract".equals(contractType)) {
				...
				} else if ("SmartContract".equals(contractType)) {
